...
-- Reads C4D (not sure about Redshift in other DCCs) Redshift formatted EXR channels
    ...
    for line in output:gmatch("[^\r\n]+") do
        ...
        if channel_list then
            for channel in channel_list:gmatch("[^,]+") do
                ...
                if base_name and not (base_name == "R" or base_name == "G" or base_name == "B" or base_name == "A" or base_name:match("^Crypt")) then
...
function output_selected_channel(file_path, oiio_path, channel_name_view, filtered_channel_name, out_path, in_path, has_alpha_channel, fileNameWithoutNumericSequence, directory)
    ...
    if has_alpha_channel then
        ...
        send_end = ".png"
        command = string.format('%s -v -i "%s" --ch "%s.red,%s.green,%s.blue,%s.alpha" --chnames "R=%s.red,G=%s.green,B=%s.blue,A=%s.alpha" --quality 100 -d uint16 --resize 0x1080 -o "%s"', oiio_path, in_path, channel_name_view, channel_name_view, channel_name_view, channel_name_view, channel_name_view, channel_name_view, channel_name_view, channel_name_view, out_path)
    else
        out_path = out_path .. ".jpg"
        send_end = ".jpg"
        command = string.format('%s -v -i "%s" --ch "%s.red,%s.green,%s.blue" --quality 100 --resize 0x1080 -o "%s"', oiio_path, in_path, channel_name_view, channel_name_view, channel_name_view, out_path)
    end
...
function output_selected_channel_lut(file_path, oiio_path, channel_name_view, filtered_channel_name, out_path, in_path, has_alpha_channel, fileNameWithoutNumericSequence, directory, ocio_config, ocio_colorspace, ocio_display, ocio_view)
    ...
    if has_alpha_channel then
        ...
        send_end = ".png"
        print(ocio_config)
        command = string.format('%s -v -i "%s" --ch "%s.red,%s.green,%s.blue,%s.alpha" --chnames "R=%s.red,G=%s.green,B=%s.blue,A=%s.alpha"  --colorconfig "%s" --iscolorspace "%s"  --ociodisplay "%s" "%s" --quality 100 -d uint16 --resize 0x1080 -o "%s"', oiio_path, in_path, channel_name_view, channel_name_view, channel_name_view, channel_name_view, channel_name_view, channel_name_view, channel_name_view, channel_name_view, ocio_config, ocio_colorspace, ocio_display, ocio_view, out_path)
    else
        out_path = out_path .. ".jpg"
        send_end = ".jpg"
        command = string.format('%s -v -i "%s" --ch "%s.red,%s.green,%s.blue" --colorconfig "%s" --iscolorspace "%s"  --ociodisplay "%s" "%s" --quality 100 --resize 0x1080 -o "%s"', oiio_path, in_path, channel_name_view, channel_name_view, channel_name_view, ocio_config, ocio_colorspace, ocio_display, ocio_view, out_path)
    end
...
-- Command formats
function exr_breakup_redshift_normal()
    local is_ocio = false
...
function exr_breakup_redshift_aces_srgb()
    local is_ocio = true
    local expanded_home_path = mp.command_native({"expand-path", "~~home/"})
    local normalized_home_path = expanded_home_path:gsub("\\", "/")
...
function exr_breakup_redshift_aces_r709()
    local is_ocio = true
    local expanded_home_path = mp.command_native({"expand-path", "~~home/"})
    local normalized_home_path = expanded_home_path:gsub("\\", "/")
    local ocio_config = normalized_home_path .. "/luts/ACES/config.ocio"
...
function exr_breakup_redshift_agx_srgb()
    local is_ocio = true
    local expanded_home_path = mp.command_native({"expand-path", "~~home/"})
    local normalized_home_path = expanded_home_path:gsub("\\", "/")
...
function exr_breakup_redshift_agx_r709()
    local is_ocio = true
    local expanded_home_path = mp.command_native({"expand-path", "~~home/"})
    local normalized_home_path = expanded_home_path:gsub("\\", "/")
    local ocio_config = normalized_home_path .. "/luts/AGX/config.ocio"
...
function exr_breakup_redshift_linear_srgb()
    local is_ocio = true
    local expanded_home_path = mp.command_native({"expand-path", "~~home/"})
    local normalized_home_path = expanded_home_path:gsub("\\", "/")
    local ocio_config = normalized_home_path .. "/luts/AGX/config.ocio"
...
function exr_breakup_redshift_linear_r709()
    local is_ocio = true
    local expanded_home_path = mp.command_native({"expand-path", "~~home/"})
    local normalized_home_path = expanded_home_path:gsub("\\", "/")
    local ocio_config = normalized_home_path .. "/luts/AGX/config.ocio"
...
-- Bindings
mp.add_key_binding("", "exr_breakup_redshift_normal", exr_breakup_redshift_normal)
mp.add_key_binding("", "exr_breakup_redshift_aces_srgb", exr_breakup_redshift_aces_srgb)
mp.add_key_binding("", "exr_breakup_redshift_aces_r709", exr_breakup_redshift_aces_r709)
mp.add_key_binding("", "exr_breakup_redshift_agx_srgb", exr_breakup_redshift_agx_srgb)
mp.add_key_binding("", "exr_breakup_redshift_agx_r709", exr_breakup_redshift_agx_r709)
mp.add_key_binding("", "exr_breakup_redshift_linear_srgb", exr_breakup_redshift_linear_srgb)
mp.add_key_binding("", "exr_breakup_redshift_linear_r709", exr_breakup_redshift_linear_r709)
