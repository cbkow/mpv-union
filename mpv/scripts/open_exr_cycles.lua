...
-- Reads Blender Cycles formatted EXR channels
    ...
    for line in output:gmatch("[^\r\n]+") do
        ...
        if channel_list then
            for channel in channel_list:gmatch("[^,]+") do
                local base_name = channel:match("ViewLayer%.([^%.]+)")
                if base_name then
                    base_name = base_name:gsub("^%s*(.-)%s*$", "%1") -- Trim spaces
                    print("Base Name: " .. base_name)
                    
                    if not (base_name == "R" or base_name == "G" or base_name == "B" or base_name:match("^Crypt")) then
                        channels[base_name] = {
    ...
    if file_path:match("%.exr$") then
        ...
        input.get_user_input(function(line)
            ...
            if choice and channel_table[choice] then
                ...
                local channel_name_view = "ViewLayer." .. filtered_channel_name
                ...
                local has_alpha_channel = output:match("ViewLayer%." .. filtered_channel_name .. "%.A") ~= nil
...
-- Command formats
function exr_breakup_cycles_normal()
    local is_ocio = false
...
function exr_breakup_cycles_aces_srgb()
    local is_ocio = true
    local expanded_home_path = mp.command_native({"expand-path", "~~home/"})
    local normalized_home_path = expanded_home_path:gsub("\\", "/")
...
function exr_breakup_cycles_aces_r709()
    local is_ocio = true
    local expanded_home_path = mp.command_native({"expand-path", "~~home/"})
    local normalized_home_path = expanded_home_path:gsub("\\", "/")
    local ocio_config = normalized_home_path .. "/luts/ACES/config.ocio"
...
function exr_breakup_cycles_agx_srgb()
    local is_ocio = true
    local expanded_home_path = mp.command_native({"expand-path", "~~home/"})
    local normalized_home_path = expanded_home_path:gsub("\\", "/")
...
function exr_breakup_cycles_agx_r709()
    local is_ocio = true
    local expanded_home_path = mp.command_native({"expand-path", "~~home/"})
    local normalized_home_path = expanded_home_path:gsub("\\", "/")
    local ocio_config = normalized_home_path .. "/luts/AGX/config.ocio"
...
function exr_breakup_cycles_linear_srgb()
    local is_ocio = true
    local expanded_home_path = mp.command_native({"expand-path", "~~home/"})
    local normalized_home_path = expanded_home_path:gsub("\\", "/")
    local ocio_config = normalized_home_path .. "/luts/AGX/config.ocio"
...
function exr_breakup_cycles_linear_r709()
    local is_ocio = true
    local expanded_home_path = mp.command_native({"expand-path", "~~home/"})
    local normalized_home_path = expanded_home_path:gsub("\\", "/")
    local ocio_config = normalized_home_path .. "/luts/AGX/config.ocio"
...
-- Bindings
mp.add_key_binding("", "exr_breakup_cycles_normal", exr_breakup_cycles_normal)
mp.add_key_binding("", "exr_breakup_cycles_aces_srgb", exr_breakup_cycles_aces_srgb)
mp.add_key_binding("", "exr_breakup_cycles_aces_r709", exr_breakup_cycles_aces_r709)
mp.add_key_binding("", "exr_breakup_cycles_agx_srgb", exr_breakup_cycles_agx_srgb)
mp.add_key_binding("", "exr_breakup_cycles_agx_r709", exr_breakup_cycles_agx_r709)
mp.add_key_binding("", "exr_breakup_cycles_linear_srgb", exr_breakup_cycles_linear_srgb)
mp.add_key_binding("", "exr_breakup_cycles_linear_r709", exr_breakup_cycles_linear_r709)
